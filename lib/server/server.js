...
var ViewDbSocketServer = function (viewdb, socket, queryDecorator) {
  ...
  socket.on('/vdb/request', function (request) {
    if (request.p.find) {
      ...
      var skip = request.p.skip;
      var limit = request.p.limit;
      var observeId = request.p.id;
      _queryDecorator(request.p.collection, request.p.observe, function (decoratedQuery) {
        delete decoratedQuery.skip;
        delete decoratedQuery.limit;
        var observeCursor = viewdb.collection(request.p.collection).find(decoratedQuery);
        if (skip) {
          observeCursor = observeCursor.skip(skip);
        }
        if (limit) {
          observeCursor = observeCursor.limit(limit);
        }
        var observeHandle = observeCursor.observe({
