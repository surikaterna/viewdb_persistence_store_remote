...
HybridCursor.prototype._count = function (callback) {
  var self = this;
  var localCount = null;
  ...
  function localResult(err, count) {
    ...
    localCount = count;
    if (remoteCount) {
      callback(null, remoteCount);
    } else {
      if (self._options.localFirst) {
        callback(err, localCount);
      }
    }
  }

  this._local.count(serverResult);
  this._remote.count(localResult);
};

HybridCursor.prototype.count = function (callback) {
  var self = this;
...
HybridCursor.prototype.observe = function (options) {
  ...
  return new Observe(
    ...
    this._options,
    options,
    this._onObserverCacheUpdate.bind(this),
    this._getObserverData.bind(this)
  );
};
